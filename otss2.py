...
for i in range(i0, i0 + n):
    # Add constraints for each coordinate
    linear_terms[i] = -lam2
    for j in range(i + 1, i0 + n):
        quadratic_terms[i, j] = 2 * lam2

for i in range(N_DATA):
    o_i = df['origin'].iloc[i]
    o_i = o_i if pd.notna(o_i) else 'PDI'
    i1 = list(coords.keys()).index(o_i)
    for j in range(n):
        quadratic_terms[i, i0 + j] = dist_matrix[i1, j]
# -----------------------------------------------
